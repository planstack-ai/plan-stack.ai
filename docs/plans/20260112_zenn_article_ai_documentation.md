...
5. 実践：Plan Stackワークフロー
   ...
   - ドメインクラスタリングとしての価値
     - planに書かれた情報はドメインの群として機能
     - 同じドメインの関連ファイルを探索する時に有効
     - 例：認証系planがあれば、次の認証作業時に関連ファイル群がまとまっている
   - planの例（簡潔に）
...
1. 「AI時代のドキュメンテーション：なぜplanをcommitに含めるべきか」
2. 「Claude Codeは探索できる。でもcontextには限界がある」
3. 「AIの外部記憶としてのplan.md」
